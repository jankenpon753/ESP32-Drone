// using loop, testing PID control for stabilization
#include <Arduino.h>
#include <Wire.h>
...
#define RECEIVER_YAW 13      // Ch1
#define RECEIVER_THROTTLE 14 // Ch2
#define RECEIVER_PITCH 16    // Ch3
#define RECEIVER_ROLL 17     // Ch4
#define RECEIVER_AUX1 18     // Ch5
#define RECEIVER_AUX2 19     // Ch6
...
float kRollAngle = 0, kPitchAngle = 0, kYawRate = 0;
float accOffset[3];
float gyroOffset[3];
float angleOffset[2];
// Kalman Filters for sensor data
SimpleKalmanFilter kalmanPitch(2.5, 1.6, 0.01);
SimpleKalmanFilter kalmanRoll(2.5, 1.6, 0.01);
SimpleKalmanFilter kalmanYaw(2.5, 1.6, 0.01);
...
void serialEdit()
{
    ...
    case 'r':
        resetPID();
        Serial.println("PID values reset!");
        break;
    case 't':
        throttle = value;
        Serial.printf("Updated throttle to %.2f\n", throttle);
        break;
    case 'h':
        desiredPitchAngle = value;
        Serial.printf("Updated desired pitch angle to %.2f\n", desiredPitchAngle);
        break;
    case 'l':
        desiredRollAngle = value;
        Serial.printf("Updated desired roll angle to %.2f\n", desiredRollAngle);
        break;
    case 'y':
        desiredYawRate = value;
        Serial.printf("Updated desired yaw rate to %.2f\n", desiredYawRate);
        break;
    default:
...
void readIMUData()
{
    ...
    kYawRate = kalmanYaw.updateEstimate(mRotRate[2]);
}
...
void pid_equation(float Error, float P, float I, float D, float &PrevError, float &PrevIterm, float &PIDOutput, int i)
{
    currentTime[i] = millis();
    elapsedTime[i] = currentTime[i] - prevTime[i];
    prevTime[i] = currentTime[i];

    float Pterm = P * Error;
    float Iterm = PrevIterm + I * Error * elapsedTime[i];
    Iterm = constrain(Iterm, -100, 100);
    float Dterm = D * (Error - PrevError) / elapsedTime[i];

    PIDOutput = Pterm + Iterm + Dterm;
    PIDOutput = constrain(PIDOutput, -200, 200);
    PrevError = Error;
    PrevIterm = Iterm;
}
...
void computePID()
{
    ...
    float errorYawRate = desiredYawRate - kYawRate;

    pid_equation(errorRollAngle, kp, ki, kd, prevErrorRollAngle, integralRollAngle, rollInput, 0);
    pid_equation(errorPitchAngle, kp, ki, kd, prevErrorPitchAngle, integralPitchAngle, pitchInput, 1);
    pid_equation(errorYawRate, kp, ki, kd, prevErrorYawRate, integralYawRate, yawInput, 2);
}
...
void computePIDRate()
{
    ...
    pid_equation(errorRoll, kp, ki, kd, prevErrorRollRate, integralRollRate, rollInput, 0);
    pid_equation(errorPitch, kp, ki, kd, prevErrorPitchRate, integralPitchRate, pitchInput, 1);
    pid_equation(errorYawRate, kp, ki, kd, prevErrorYawRate, integralYawRate, yawInput, 0);
}
...
void loop()
{
    ...
    computePID();         // Compute stabilization control
    ...
    Serial.print(" Yaw:");
    Serial.print(mRotRate[2]);
    Serial.print(" M1:");
    Serial.print(motorInputs[0]);
    Serial.print(" M2:");
    Serial.print(motorInputs[1]);
    Serial.print(" M3:");
    Serial.print(motorInputs[2]);
    Serial.print(" M4:");
    Serial.print(motorInputs[3]);
    // Serial.print(" Kp:");
    // Serial.print(kp);
    // Serial.print(" Ki:");
    // Serial.print(ki);
